...
## Agent memory
三种记忆机制
Semantic（语义记忆） 存储"是什么"的知识，比如"巴黎是法国首都"。在 LangChain 里对应 RAG 中的向量检索，Agent 从外部知识库查事实。
Episodic（情节记忆） 存储"发生过什么"的经历，比如"上次用户问过 XX"。对应 LangChain 里的 ConversationBufferMemory、ConversationSummaryMemory 等，保存对话上下文。
Procedural（程序记忆） 存储"怎么做"的方法，比如骑自行车不需要思考就会。对应 System Prompt 里写的行为规范、以及 Agent 学会调用哪些工具、以什么方式响应——这部分通常是"固化"在模型或配置里的。


## 更新记忆的方式
hot path：实时更新
background：异步更新，记忆更新与核心response解耦，一段时间后更新
