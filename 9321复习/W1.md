...
- 现代数据系统依赖多个协同组件（cache／DB／search／queue）。
    
- 这样可以满足不同需求：高性能、搜索能力、异步处理等。
	    
- 应用代码充当调度者，通过抽象层统一调用这些组件。
    
- 最终实现：**高性能、可扩展、结构化的数据导向服务架构**。
### **📌 Information Systems / Application Integration**

- 目标：让不同来源、不同年代的系统可以互相协作。
    
- 动机：精简流程、竞争压力、系统合并、全球化。
    
- 问题：系统高度异构，技术、平台、数据结构不一致。


## 🔍 1. 什么是 Data Integration？

	PPT 提供的定义：

**从不同系统（不同数据源）收集数据 → 转换 → 合并 → 提供一个统一的视图**




## ==🔍 3. Local Schema vs Global Schema（重点概念）==

### **Local Schema（本地模式）**

- 每个数据源都有自己独立的数据结构（字段名字不同、格式不同）。
    
- 如 Source 1 的 OrderID、Source 2 的 CustomerID。
    

### **Global Schema（全局模式）**

- Mediator（中介器）构建一个 **统一的虚拟模式**，把所有来源映射到一份标准化结构。
    
- 用户只看到 Global Schema，不需要关心原来数据来源。
...
- **读取 local schemas 的数据**
    
- **转换成统一格式**
    
- **关联彼此（如用 CustomerID 连接 Source1 和 Source3）**
    
- 最终生成一个 Integrated Order DB（集成后的数据库）

---

## 🎯 这页的核心思想

- Data Level Integration = 合并不同数据源，构建统一视图
    
- 每个系统有 Local Schema
    
- Mediator 使用 Global Schema 将它们整合
...
Data Level Integration（VDIS Architecture）解释
**原数据**（高度异构）
**包装器层**（为每个数据源“翻译”成统一格式的适配器）
## **Mediator（中介层）**
...
### **1. 存在两种主要的数据集成映射方式：GAV 与 LAV**

- **GAV（Global-As-View）**：  
    全局模式由多个本地数据源组合而成。  
    ➜ _Global schema = view over local schemas_
    
- **LAV（Local-As-View）**：  
    本地数据源由全局模式的视图定义。  
    ➜ _Local schema = view over global schema_
    

👉 **核心目的：解决不同来源的数据字段不一致、结构不同的问题。**

---

# ✅ **核心要点（图 2：Materialized vs Virtual Integration）**

### **2. 数据集成有两种主要实现方式：物化集成与虚拟集成**

- **Materialized Integration（物化集成）**
    
    - 数据从各个数据源抽取进 Data Warehouse
        
    - 使用 ETL（Extract-Transform-Load）
...
- **Virtual Integration（虚拟集成）**
    ...
    - 通过 Wrapper + Mediator 在查询时实时访问各数据源
        
    - 使用 Global Schema 提供单一视图
...
👉 **核心目的：为用户提供统一的数据接口，不管数据实际存在哪里。**




**跨系统、跨平台的交互比内部集成困难得多，需要标准化的通信方式。**
软件架构正在从代码级抽象转向服务级抽象，服务化是现代系统集成的关键。



SOA（Service-Oriented Architecture）强调：软件系统由 **多个可独立部署、独立维护的服务** 组成
**服务是现代软件的基础抽象，强调独立、契约化、可复用和可组合。**

服务通过标准接口（API）互相调用，形成可组合的服务网络，是系统集成的关键方式。