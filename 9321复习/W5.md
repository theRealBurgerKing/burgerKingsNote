...
def verify_signature(request):
    ...
    # 重构消息
    message = f"{api_key}{timestamp}{request.body}"
...
### 2. Query Parameters（不推荐）

python

```python
GET /api/data?api_key=abc123&secret=xyz789
```
...
def check_rate_limit(api_key, limit=5, window=60):
    ...
    request_counts[api_key] = [
        timestamp for timestamp in request_counts[api_key]
        if timestamp > cutoff
    ]
...
def get_data():
    ...
    allowed, status = check_rate_limit(api_key, limit=5, window=60)
    if not allowed:
...
def log_api_call(api_key, endpoint, status_code):
    api_usage['calls'][api_key] += 1
    api_usage['endpoints'][api_key][endpoint] += 1
    
    if status_code >= 400:
        api_usage['errors'][api_key] += 1

# 生成报告
def generate_usage_report(api_key):
    return {
        'total_calls': api_usage['calls'][api_key],
        'endpoints': dict(api_usage['endpoints'][api_key]),
        'error_count': api_usage['errors'][api_key]
    }
```