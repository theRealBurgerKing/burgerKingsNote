...
> 任何安全敏感信息的REST API都应该使用TLS（替代了旧的SSL）
...
GET /secret.html HTTP/1.1
Host: example.org
Authorization: Basic QmVuOmNibkNiNk3xNJ3xCg==
```

**后续请求**：

- 相同或更深路径的请求可以==预先包含Authorization头==

==Authorization就是base64编码的”用户名：密码“==
...
**问题3：不够安全**
> HTTP Basic Auth结合TLS在某些情况下可以工作...但通常这个方案不被推荐，被认为不够"安全"
...
这是现代 API 认证的主流方案，比 HTTP Basic Auth 安全得多。让我详细解释：
...
## ==JWT (JSON Web Tokens) - 行业标准==

==JWT 是目前最流行的 Token 实现方式（RFC 7519 标准）==
（仅作为扩展，不必了解）
### JWT 的三部分结构

一个完整的 JWT 看起来像这样：
```
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTYiLCJuYW1lIjoiSm9obiBTbWl0aCIsImVtYWlsIjoiai5zbWl0aEB4eXouY29tIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c
````
...
```
**Base64 编码后：** `eyJzdWIiOiIxMjM0NTYiLCJuYW1lIjoiSm9obiBTbWl0aCIsImVtYWlsIjoiai5zbWl0aEB4eXouY29tIn0`
...
- 在所有请求中复制发出的API key (和secret)

**3. 认证目的**

- ≈ 用户ID和密码，但它是用来认证"客户端应用"
...
> API key（结合Secret）→ 一种认证方案

**主要用途**：

> 通常，API key让你可以访问同一提供商的广泛服务
...
- 例如：每秒5次调用 | 429 "Too Many Requests"
- 防止滥用
...
> 用户如何允许公司B访问公司A中的数据，而不向公司B泄露公司A的登录凭证？

比如我想要谷歌邮箱的数据来获取facebook 可能认识的人
...
```
❌ 用户必须把Gmail密码给Facebook
Facebook用这个密码访问Gmail
风险：Facebook知道了你的Gmail密码
```
...
2. 用户在Gmail登录并授权
   → Gmail: "Facebook想访问你的联系人，允许吗？"

3. Gmail返回授权码给Facebook
   → 授权码：一次性使用的临时码

4. Facebook用授权码换取访问令牌
   → 访问令牌：可以访问资源的凭证

5. Facebook用访问令牌访问Gmail API
   → 获取联系人列表
```
...
- Google 需要知道谁是合法的第三方应用

**2. A为B签发'API key'和Secret（用于B的认证）**

- B获得客户端凭证

**3. 数据访问授权（每个客户）通过OAuth完成**

- 用户级别的授权通过OAuth流程（就是上面提到的过程）
...
## ==✅ **What else you need to know about REST API security?（还需要了解的 REST API 安全点）**==

这一页列出开发 REST API 时需要注意的常见安全措施：

- **Rate limiting（限流）**：限制客户端请求速度，避免过度使用资源。
    
- **Restrict HTTP methods（限制 HTTP 方法）**：只允许必要的方法，例如只开放 GET/POST，禁止 PUT/DELETE 等危险操作。
    
- **Input validation（输入校验）**：防止注入攻击（如 SQL 注入、XSS）。
    
- **Sensitive information in HTTP requests（敏感信息处理）**：敏感数据不要明文传输，也不要放在 URL 中。
    
- **Audit logs（审计日志）**：记录访问行为，方便排查问题或异常活动。
    
- **Send appropriate error logs（适当的错误日志）**：给用户返回安全的错误信息，同时系统内部记录详细错误。
...
- **Prevent farming（防止刷接口）**：避免有人大量调用 API 消耗你的成本。
    
- **Control DDoS attacks（控制潜在 DDoS 攻击）**：避免服务器因过量请求崩溃。
    

### **API Keys 的作用**

- **Require API keys**：每个请求都必须带 API Key，才能访问受保护的接口。
    
- **Return 429 Too Many Requests**：当请求过快时，返回 HTTP 429 提醒客户端限流。
    
- **Revoke API key**：如果用户滥用 API，可直接吊销其 Key。
    

**PPT 要传达的重点：限流是保护 API 的核心策略，API Key 和 429 响应是常见做法。**


# ✅ **Restrict HTTP methods（限制 HTTP 方法）**

这一页主要讲：API 不应该随便开放所有 HTTP 方法。

- **Do consumers need all methods?**  
    检查客户端是否真的需要用到所有 HTTP method。
    
- **Use whitelist（白名单）**  
    只允许指定的方法，例如 GET、POST、PUT。
    
- **Return 405 for disallowed methods**  
    不在白名单中的方法全部拒绝，并返回 **405 Method Not Allowed**。
    
- **Authorization per method**  
    确保调用方有权限使用某个 HTTP 方法访问对应的资源或操作。
    

**核心意思：只开放需要的方法，其他全部禁止，并确保调用者有权限执行该方法。**
...
- **Strong typing helps**  
    使用强类型（数字、布尔、日期等）让输入更可控。
    
- **Regex for strings**  
    用正则限制字符串输入。
    
- **Reject illegal content**  
    遇到非法或异常内容要拒绝。
    
- **Use validation libraries**  
    建议使用 Python 的验证库（例如 Validator Collection）。
    

**核心意思：输入必须严格验证，否则容易被注入或导致错误行为。**

---

# ✅ **Sensitive information in HTTP requests（处理 HTTP 中的敏感信息）**

主题：敏感信息不要出现在 URL 里。

- **Sensitive data should NOT appear in URL**  
    URL 会被日志系统记录，非常不安全。
    
- **POST/PUT：放在 body 或 header**  
    敏感信息要放在请求体或头部。
    
- **GET：放在 header，而不是 query string**  
    GET 的 query 参数不安全，所以敏感信息也要放 header。
    
- **Example（不安全写法）**
    
    `https://example.com/controller/123/action?apiKey=a53f435643de32`
    
    API key 暴露在 URL → 非常不安全。
    

**核心意思：敏感信息必须通过 Header 或 Body 传输，不要出现在 URL/query string。**
...
- **Logs help detect incidents quickly**  
    日志可以帮助快速发现安全事件。
    
- **Write logs before and after security-related events**  
    安全相关操作的前后都应该写日志。
    
- **Log token validation errors**  
    token 验证失败的日志有助于发现攻击（例如大量无效 token 请求）。
    
- **Sanitize log data to avoid log injection**  
    记录前要对数据做清洗，避免日志注入攻击。
    

**核心意思：日志是检测攻击的重要工具，但日志本身也必须安全处理。**

---

# ✅ **Send Appropriate Error logs（发送合适的错误信息）**

这一页说明错误信息应该如何返回给客户端：

- **Use correct status codes**  
    返回语义正确的 HTTP 状态码（例如 400、401、403、500）。
    
- **Generic error messages**  
    给客户端的错误提示要模糊，不要泄露系统细节。
    
- **Do not include internal technical details**  
    不要把堆栈信息、内部路径、数据库信息返回给用户。
    

**核心意思：错误应该“够用但不暴露内部结构”。**
...
# ✅ **How can I test my REST API（如何测试 REST API 的安全性）**

这一页讲如何做安全测试：

- **Many tools can scan REST APIs**  
    有许多 web 安全测试工具也可以扫描 REST API。
...
- **Be ethical**  
    只测试你**自己拥有或被授权的**系统，不要随便扫描别人。
    

**核心意思：使用常见安全工具主动测试自己的 API，但必须遵守伦理和授权范围。**