...
这张幻灯片讨论 **'static lifetime（静态生命周期）** 🐋，提出三个问题。
...
- 字符串字面量存储在**二进制文件的只读数据段**
- 在程序运行期间**始终有效**
- 类型是 `&'static str`（静态生命周期的字符串切片）
...
let r = &GLOBAL;  // r 的类型是 &'static i32
```

**答案：** 对全局变量（`static` 或 `const`）的引用也是 `'static`

---

## 'static 的含义

`'static` 表示数据在**整个程序运行期间**都有效。


## 常见误区

❌ **错误理解：** `'static` 表示"永远不变"  
✅ **正确理解：** `'static` 表示"整个程序期间都有效"
